...
const emptyProduct = (): Partial<Product> => ({
  name: '', price: 0, categoryId: '', subcategoryId: '', sizes: [], colors: [], description: '', image: '',
});
...
const AdminProducts = () => {
  ...
  const subcategories = form.categoryId ? categories.find(c => c.id === form.categoryId)?.subcategories || [] : [];
  ...
  const addColor = () => {
    if (colorName && !form.colors?.find(c => c.name === colorName)) {
      setForm(prev => ({ ...prev, colors: [...(prev.colors || []), { name: colorName, hex: colorHex, inStock: true }] }));
  ...
  const handleImageUpload = async (file: File) => {
    ...
    setUploading(true);
    try {
      // Tenta upload pro Cloudinary
      const result = await imageService.upload(file);
      setForm(prev => ({ ...prev, image: result.secure_url }));
      toast({ title: 'Imagem enviada com sucesso!' });
    } catch {
      // Fallback: preview local (dev sem Cloudinary configurado)
      console.warn('Cloudinary não configurado, usando preview local.');
      const localUrl = await imageService.previewLocal(file);
      setForm(prev => ({ ...prev, image: localUrl }));
      toast({ title: 'Preview local carregado', description: 'Configure o Cloudinary para upload permanente.' });
    } finally {
      setUploading(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      if (editing) {
        ...
        setProductsList(prev => prev.map(p => p.id === editing.id ? { ...editing, ...form } as Product : p));
        toast({ title: 'Produto atualizado com sucesso!' });
      } else {
        let newP: Product;
        try {
          newP = await productService.create(form as Omit<Product, 'id'>);
        } catch {
          // Fallback local
          newP = { ...form, id: Date.now().toString() } as Product;
        }
          ...
          <DialogContent className="max-h-[90vh] overflow-y-auto max-w-2xl">
            ...
            <div className="space-y-4">
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div><Label>Preço *</Label><Input type="number" step="0.01" value={form.price} onChange={e => setForm(p => ({ ...p, price: parseFloat(e.target.value) || 0 }))} /></div>
                <div>
